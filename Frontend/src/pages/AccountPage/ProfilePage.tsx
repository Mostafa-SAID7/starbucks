...
const AdminDashboard: React.FC<{ user: any }> = ({ user }) => {
  ...
  const adminLinks = [
    { title: t('pages:profile.admin.users'), count: '124', color: 'bg-blue-500' },
    { title: t('pages:profile.admin.categories'), count: '12', color: 'bg-green-500' },
    { title: t('pages:profile.admin.menu_items'), count: '86', color: 'bg-amber-500' },
    { title: t('pages:profile.admin.analytics'), count: t('pages:profile.admin.daily_reports'), color: 'bg-purple-500' },
  ];
  ...
  return (
    <div className="space-y-6">
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
        {adminLinks.map((link) => (
          <Card key={link.title} className="p-6 hover:shadow-md transition-shadow cursor-pointer border-t-4" style={{ borderTopColor: link.color.replace('bg-', '') }}>
