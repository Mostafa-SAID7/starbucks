...
import { Search, ChevronRight, ChevronLeft, ExternalLink, Coffee } from 'lucide-react'
      ...
      <div className="mt-8 min-h-[300px]">
        <AnimatePresence mode="wait">
          {!searchTerm.trim() ? (
            ...
            >
              <p className={`text-xs font-black uppercase tracking-[0.2em] text-gray-400 mb-4 ${isRTL ? 'text-right' : 'text-left'}`}>
                {searchResults.length > 0 
                  ? (lang === 'ar' ? `النتائج (${searchResults.length})` : `Results (${searchResults.length})`) 
              ...
              {searchResults.map((item, index) => (
                <motion.a
                  ...
                  onClick={onClose}
                  className={`group flex items-center gap-4 p-4 rounded-xl hover:bg-gray-50 dark:hover:bg-zinc-800/50 transition-colors border border-transparent hover:border-gray-200 dark:hover:border-zinc-700 ${isRTL ? 'text-right flex-row-reverse' : 'text-left'}`}
                >
                  ...
                  <div className="flex-grow">
                    <h4 className="font-bold text-gray-900 dark:text-white group-hover:text-starbucks-green transition-colors">{item.title}</h4>
                    <p className="text-sm text-gray-500 dark:text-gray-400 line-clamp-1">{item.description}</p>
                    <div className={`flex items-center gap-2 mt-1 text-xs text-gray-400 ${isRTL ? 'flex-row-reverse' : ''}`}>
                  ...
                  <div className="flex-shrink-0 text-gray-300 group-hover:text-starbucks-green transition-colors">
                    {item.href.startsWith('http') ? <ExternalLink className="w-5 h-5" /> : (isRTL ? <ChevronLeft className="w-5 h-5" /> : <ChevronRight className="w-5 h-5" />)}
                  </div>
                </motion.a>
              ))}
            </motion.div>
